...
5. **Copy the plugin to your vault**:
   ...
   **Note**: The `npm run build` command only copies `main.js` and `styles.css` if you have configured the copy path in `package.json`. You still need to manually copy `manifest.json` and the `media/` folder on first installation.
...
2. Bundles the plugin with esbuild
3. Automatically copies `main.js` and `styles.css` to your vault (if configured in `package.json`)

**First-time setup**: You need to manually copy `manifest.json` and the `media/` folder to `.obsidian/plugins/vault-pets/` in your vault. After that, `npm run build` will update the JavaScript and CSS files automatically.
